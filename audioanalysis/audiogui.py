...
def async_gui_call(fn):
    """Decorator for asynchronous AudioGUI functions"""
...
class AudioGUI(Ui_MainWindow, QMainWindow):
    ...
    def __init__(self):
        ...
        defaultlayers = [
            {'type': 'Convolution2D', 'args': (
                16, 3, 1,), 'kwargs': {'border_mode': 'same'}},
        ...
        self.play_button.setIcon(
            QtGui.QIcon(os.path.join(sysdir, 'icons/play.png')))
    ...
    def busy_alert(self):
        ...
        for t in self.threads:
            self.logger.warning(
                'Thread {0} is running {1}'.format(id(t), t.name))
    ...
    def select_folder_callback(self):
        ...
        if folder_name:
            files = self.find_files(
                str(folder_name), '*.wav') + self.find_files(str(folder_name), '*.WAV')
    ...
    def table_clicked_async(self, table, row):
        ...
        if table == 'motifs':
            ...
            self.logger.warning(
                'Unknown table %s clicked, cannot display song', table)
    ...
    def save_neural_net_callback(self):
        """Save the analyzer's current neural net to a file"""
    ...
    def play_audio(self, in_data, frame_count, time_info, status):
        ...
        self.canvas.set_marker(
            (index + frame_count) / self.analyzer.active_song.Fs)
    ...
    def plot(self, plot_type):
        ...
        try:
            ...
            self.logger.warning(
                'Missing time domain display downsampling ratio')
            t_step = 1
        ...
        try:
            ...
            self.logger.warning(
                'Missing frequency domain display downsampling ratio')
            f_step = 1
        ...
        try:
            ...
            disp_Sxx = 10 * \
                np.log10(np.flipud(self.analyzer.Sxx[::t_step, ::f_step]))
        except AttributeError:
            self.logger.warning(
                'No active song, cannot display plot %s', plot_type)
            return

        if plot_type == 'spectrogram':
            self.canvas.display_spectrogram(
                time, freq, disp_Sxx, title=title, **self.params)
    ...
    def auto_classify_async(self, mode):
        ...
        try:
            ...
            self.logger.error(
                'No neural net yet trained, cannot classify songs')
        else:
            self.logger.info('Took {0:2.3f} seconds to classify {1} songs'.format(
...
class SpectrogramCanvas(FigureCanvas, QtCore.QObject):
    ...
    within this class should emit a signal which should be bound to the
    ...
    def add_tool(self, tool):
        """Add the tool to the tools dictionary and connect any known signal"""
    ...
    def navigate(self, kwargs):
        ...
        if navtype == 'drag_pan':
            try:
                ...
                self.logger.error(
                    'Invalid drag command or signal, cannot execute')
                return
            ...
            try:
                ...
                self.logger.warning(
                    "Scrolling irrelevant, no plots at this time")
            else:
                xbounds = ax.get_xlim()
    ...
    def set_domain(self, x_domain):
        ...
        for name in self.axis_names:
            ...
            self.logger.debug(
                'Setting domain for axis %s to %s', name, str(x_domain))
...
class SpectrogramNavBar(NavigationToolbar2QT):
    ...
    def __init__(self, canvas_, parent_, *args, **kwargs):
        ...
        NavigationToolbar2QT.__init__(
            self, canvas_, parent_, coordinates=False)

        self.custom_toolitems = (
            (None, None, None, None),
            ('Select', 'Cursor with click, select with drag',
             'select1', 'select'),
        )
    ...
    def drag_pan(self, event):
        ...
        for a, _ in self._xypress:
            #  safer to use the recorded button at press than current button
    ...
    def select(self, *args):
        ...
        Select with left button, set cursor with right
        """
    ...
    def _set_cursor(self, event):
        """OVERRIDE the _set_cursor method in NavigationToolbar2"""
